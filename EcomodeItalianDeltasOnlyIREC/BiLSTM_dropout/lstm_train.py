...
class Configuration(object):
	...
	output_keep_prob=float(sys.argv[7])
...
def train():
	...
	filename_train=['/home/local/IIT/rtavarone/Data/PreProcessEcomodeItalian/DELTAS_TFRECORDS/TRAIN_FEAT_IREC_AUGMENT_TFRECORDS/sequence_full_{:04d}.tfrecords'.format(i) \
					 for i in range(num_examples)]
	for f in filename_train:
		if not tf.gfile.Exists(f):
			raise ValueError('Failed to find file: ' + f)

	filename_val=['/home/local/IIT/rtavarone/Data/PreProcessEcomodeItalian/DELTAS_TFRECORDS/VAL_FEAT_IREC_TFRECORDS/sequence_full_{:04d}.tfrecords'.format(i) \
	...
	with tf.Graph().as_default():
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
			...
			trainingLogFile.write('## Start time = {} \n'.format(time.asctime( time.localtime(time.time()) )))
			...
			print('## output_keep_prob : ',config.output_keep_prob)
			trainingLogFile.write('## output_keep_prob : {:.6f} \n'.format(config.output_keep_prob))
			...
			try:
				...
				while not coord.should_stop():
					...
					_ , C  = sess.run([train_model.optimize,train_model.cost])
					...
					if ((step % int(num_examples / config.batch_size) == 0) and (step is not 0)):
						...
						best_epoch_index=val_accuracy_list.index(max(val_accuracy_list))
						if epoch_counter >= (best_epoch_index+7):
							print('Accuracy not increased for the last 7 epochs. Exit.')

							print('## End time = {}'.format(time.asctime( time.localtime(time.time()) )))
							trainingLogFile.write('## End time = {} \n'.format(time.asctime( time.localtime(time.time()) )))
