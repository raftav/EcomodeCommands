...
def train():
	...
	filename_train=['/home/local/IIT/rtavarone/Data/PreProcessEcomodeItalian/DELTAS_TFRECORDS/TRAIN_FEAT_INEC_AUGMENT_TFRECORDS/sequence_full_{:04d}.tfrecords'.format(i) \
					 for i in range(num_examples)]
	for f in filename_train:
		if not tf.gfile.Exists(f):
			raise ValueError('Failed to find file: ' + f)

	filename_val=['/home/local/IIT/rtavarone/Data/PreProcessEcomodeItalian/DELTAS_TFRECORDS/VAL_FEAT_INEC_TFRECORDS/sequence_full_{:04d}.tfrecords'.format(i) \
			...
			try:
				...
				while not coord.should_stop():
					...
					if ((step % int(num_examples / config.batch_size) == 0) and (step is not 0)):
						...
						if epoch_counter >= (best_epoch_index+5):
							print('Accuracy not increased for the last 5 epochs. Exit.')
