...
checkpoints_dir='/home/local/IIT/rtavarone/Ecomode/Ecomode_LSTM/checkpoints/exp'+str(ExpNum)+'/'
...
def save_graph():
	...
	with tf.Graph().as_default():
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
			...
			save_path = saver.save(sess,'/home/local/IIT/rtavarone/Ecomode/Ecomode_LSTM_Export_1/model_exp'+str(ExpNum)+'_epoch'+str(restore_epoch)+'.ckpt')
			tf.train.write_graph(sess.graph_def,'/home/local/IIT/rtavarone/Ecomode/Ecomode_LSTM_Export_1/', 'model_exp'+str(ExpNum)+'_epoch'+str(restore_epoch)+ '.pbtxt')
